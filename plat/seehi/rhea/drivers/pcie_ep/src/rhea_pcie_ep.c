...
static int __unused dw_pcie_ep_set_msix(uint64_t dbi_base, uint32_t interrupts, uint32_t bar_offset, uint32_t bir)
{
	uint8_t offset = 0xb0;
	uint32_t val, reg;
...
static int __unused dw_pcie_ep_set_msi(uint64_t dbi_base, uint32_t interrupts)
{
	uint32_t val, reg;
	uint8_t offset = 0x50;
...
HAL_Status PCIe_EP_Init(struct HAL_PCIE_HANDLE *pcie)
{
	...
	uint64_t apb_base = pcie->dev->apbBase;
	uint64_t __unused ss_base = pcie->dev->ssBase;
	uint32_t __unused dniu_base = 0x11002000;
	uint32_t __unused mbitx_ap_base = 0x10050000;
